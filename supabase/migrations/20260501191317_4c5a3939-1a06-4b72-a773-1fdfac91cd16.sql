...
RETURNS TABLE(
  ...
  revenue_per_lead numeric, consistency_score numeric, activity_score numeric, hygiene_score numeric,
  talent_score numeric, talent_category text, primary_bottleneck text, bottleneck_confidence numeric,
...
BEGIN
  ...
  -- Real 7d trend: compare current 7d vs previous 7d closes
  trend_cte AS (
    SELECT COALESCE(fl.closer_id, fl.owner_id) AS op_id,
      COUNT(*) FILTER (WHERE fl.created_at >= now() - interval '7 days') AS cur7,
      COUNT(*) FILTER (WHERE fl.created_at >= now() - interval '14 days' AND fl.created_at < now() - interval '7 days') AS prev7
    FROM filtered_leads fl
    WHERE (fl.outcome = 'won' OR fl.payment_status = 'paid')
    GROUP BY 1
  ),
  combined AS (
    SELECT o.id, o.full_name, o.bstage, o.lvl, o.op_role, o.tenure,
      ...
      -- Real trend: % change in closes
      CASE WHEN COALESCE(tr.prev7, 0) = 0 THEN
        CASE WHEN COALESCE(tr.cur7, 0) > 0 THEN 100 ELSE 0 END
      ELSE ROUND(((tr.cur7 - tr.prev7)::numeric / tr.prev7) * 100, 1) END AS real_trend
    FROM operators o
    ...
    LEFT JOIN trend_cte tr ON tr.op_id = o.id
  ),
  scored AS (
    SELECT *,
      ROUND(
