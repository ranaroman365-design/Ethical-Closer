...
-- 1. Add action_source column to calendar_events
ALTER TABLE public.calendar_events
  ADD COLUMN IF NOT EXISTS action_source text NOT NULL DEFAULT 'manual';

COMMENT ON COLUMN public.calendar_events.action_source IS 'Source of action: manual, drag_and_drop, system, api';

-- 2. Update reschedule_appointment to write appointment_reschedules + action_source
...
BEGIN
  ...
  -- ★ Write to appointment_reschedules (before/after mapping)
  INSERT INTO public.appointment_reschedules (
    old_appointment_id, new_appointment_id, lead_id,
    rescheduled_by, reason, setter_id
  ) VALUES (
    _old_id, v_new_id, v_old.lead_id,
    v_caller, _reason, v_old.setter_id
  );

  -- Log to calendar_events with action_source
  INSERT INTO public.calendar_events (
    event_type, appointment_id, lead_id,
    previous_owner_id, new_owner_id, actor_user_id,
    reason, action_source, metadata
  ) VALUES (
    'appointment_rescheduled', v_new_id, v_old.lead_id,
    NULL, NULL, v_caller,
    _reason, _action_source,
    jsonb_build_object(
...
-- 3. Update reassign_appointment to accept _action_source
...
-- 4. Update reassign_and_reschedule_appointment to accept _action_source
...
BEGIN
  ...
  -- ★ Write to appointment_reschedules
  INSERT INTO public.appointment_reschedules (
    old_appointment_id, new_appointment_id, lead_id,
    rescheduled_by, reason, setter_id
  ) VALUES (
    p_appointment_id, v_new_id, v_appt.lead_id,
    v_caller, p_reason, v_appt.setter_id
  );
  ...
  -- calendar_events with action_source
  INSERT INTO public.calendar_events (event_type, appointment_id, lead_id, previous_owner_id, new_owner_id, actor_user_id, reason, action_source, metadata)
  VALUES (
    'appointment_reassigned_and_rescheduled', v_new_id, v_appt.lead_id,
    v_prev_owner_id, p_new_owner_id, v_caller, p_reason, _action_source,
    jsonb_build_object(
