...
DROP FUNCTION IF EXISTS public.compute_talent_scores(integer, uuid);
...
RETURNS TABLE(
  user_id uuid, full_name text, business_stage text, level_num integer, operator_role text,
  tenure_days integer, total_leads integer, total_bookings integer, total_shows integer,
  total_closes integer, total_revenue numeric, booking_rate numeric, show_rate numeric,
  close_rate numeric, revenue_per_lead numeric, consistency_score numeric,
  activity_score numeric, hygiene_score numeric, talent_score numeric,
  talent_category text, primary_bottleneck text, bottleneck_confidence numeric, trend_7d numeric,
...
BEGIN
  ...
  -- Build team scope array (L6 sees own team, L7 director area, L8/admin passes NULL = all)
  ...
  -- Filtered real leads (funnel applied here)
  filtered_leads AS (
    SELECT rl.*
  ...
  lead_counts AS (
    SELECT 
      COALESCE(fl.owner_id, fl.setter_id, fl.closer_id) AS op_id,
      COUNT(*) AS cnt
    FROM filtered_leads fl
    GROUP BY 1
  ),
  -- Bookings: filter by funnel via lead join
  booking_counts AS (
    SELECT 
    ...
    WHERE a.starts_at >= v_cutoff
      AND (p_funnel IS NULL OR EXISTS (
        SELECT 1 FROM filtered_leads fl WHERE fl.id = a.lead_id
      ))
    GROUP BY 1
  ),
  show_counts AS (
  ...
  close_counts AS (
    SELECT 
      COALESCE(fl.closer_id, fl.owner_id) AS op_id,
      COUNT(*) AS cnt,
      SUM(COALESCE(fl.deal_value, 0)) AS rev
    FROM filtered_leads fl
  ...
  weekly AS (
    SELECT COALESCE(fl.closer_id, fl.owner_id) AS op_id,
      EXTRACT(WEEK FROM fl.created_at) AS wk,
      COUNT(*) AS cnt
    FROM filtered_leads fl
    WHERE (fl.outcome = 'won' OR fl.payment_status = 'paid')
    GROUP BY 1,2
