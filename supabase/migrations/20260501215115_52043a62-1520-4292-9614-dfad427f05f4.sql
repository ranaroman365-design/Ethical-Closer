...
BEGIN
  ...
  result := jsonb_build_object(
    'identity', jsonb_build_object(
      ...
      'source_funnel', lead_row.source_funnel,
      'funnel_id', lead_row.funnel_id,
      'stage', lead_row.stage,
      'lead_status', lead_row.lead_status,
      'lead_level', lead_row.lead_level,
      'created_at', lead_row.created_at,
      'updated_at', lead_row.updated_at,
      'owner_name', lead_row.owner_name,
      'owner_id', lead_row.owner_id,
      'setter_name', lead_row.setter_name,
      'setter_id', lead_row.setter_id,
      'closer_name', lead_row.closer_name,
      'closer_id', lead_row.closer_id,
      'unit_id', lead_row.unit_id,
      'unit_name', v_unit_name,
      'referrer_user_id', lead_row.referrer_user_id,
      'priority_flag', lead_row.priority_flag,
      'has_booking', lead_row.has_booking,
      'booking_status', lead_row.booking_status
    ),
    'quiz', jsonb_build_object(
      'quiz_score', lead_row.quiz_score,
      'quiz_result', lead_row.quiz_result,
      'quiz_funnel_source', lead_row.quiz_funnel_source,
      'quiz_answers', lead_row.quiz_answers,
      'lead_score', lead_row.lead_score,
      'lead_quality', lead_row.lead_quality,
      'scored_at', lead_row.scored_at,
      'qualification_score', lead_row.qualification_score,
      'qualification_bucket', lead_row.qualification_bucket,
      'qualification_path', lead_row.qualification_path,
    ...
    'appointments', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'priority_price', a.priority_price,
        'rescheduled_from_id', a.rescheduled_from_id,
        'rescheduled_to_id', a.rescheduled_to_id,
        'rescheduled_at', a.rescheduled_at,
        'no_show_detected_at', a.no_show_detected_at,
        'call_started_at', a.call_started_at,
        'completed_at', a.completed_at,
        'setter_notes', a.setter_notes,
    ...
    'payments', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'status', pl.status,
        'deal_type', pl.deal_type,
        'payment_type', pl.payment_type,
        'offer_title', pl.offer_title,
        'payment_url', pl.payment_url,
        'created_at', pl.created_at,
        'sent_at', pl.sent_at,
        'opened_at', pl.opened_at,
        'paid_at', pl.paid_at,
        'expires_at', pl.expires_at,
        'refunded_at', pl.refunded_at,
        'closer_id', pl.closer_id,
        'closer_name', plc.full_name,
        'net_amount', pl.net_amount,
        'stripe_fee', pl.stripe_fee
      ) ORDER BY pl.created_at DESC)
      FROM payment_links pl
      LEFT JOIN profiles plc ON plc.id = pl.closer_id
    ...
    'commissions', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', cm.id,
        'user_id', cm.user_id,
        'user_name', cmp.full_name,
        'role', cm.role,
        'amount', cm.amount,
        'source_type', cm.source_type,
        'payout_status', cm.payout_status,
        'created_at', cm.created_at,
        'eligible_at', cm.eligible_at,
        'paid_at', cm.paid_at
      ) ORDER BY cm.created_at DESC)
      FROM commissions cm
      LEFT JOIN profiles cmp ON cmp.id = cm.user_id
      WHERE cm.call_id IN (SELECT cc.id FROM calls cc WHERE cc.lead_id = p_lead_id)
        AND NOT COALESCE(cm.is_simulation, false)
    ), '[]'::jsonb),
    ...
    'revenue', jsonb_build_object(
      ...
      'close_reason', lead_row.close_reason,
      'deal_value', lead_row.deal_value,
      'payment_status', lead_row.payment_status,
      'closed_at', lead_row.closed_at,
      'follow_up_date', lead_row.follow_up_date
    ),
    'notes', jsonb_build_object(
      'setter_notes', lead_row.setter_notes,
      'closer_notes', lead_row.closer_notes,
