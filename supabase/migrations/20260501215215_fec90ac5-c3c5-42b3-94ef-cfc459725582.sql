  ...
  result := jsonb_build_object(
    ...
    'payments', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'closer_name', plc.full_name, 'net_amount', pl.net_amount,
    ...
    'commissions', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', cm.id, 'user_id', cm.user_id, 'user_name', cmp.full_name,
        'role', cm.role, 'amount', cm.amount, 'source_type', cm.source_type,
        'payout_status', cm.payout_status, 'created_at', cm.created_at,
    ...
    'touchpoints', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'id', d.id, 'event_key', d.event_key, 'channel', d.primary_channel,
        'template_key', d.template_key, 'status', d.status,
        'dispatched_at', d.dispatched_at, 'outcome', d.outcome,
