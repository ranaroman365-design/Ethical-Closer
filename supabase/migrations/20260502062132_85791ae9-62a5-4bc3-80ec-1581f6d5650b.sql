...
BEGIN
  FOR r IN
    SELECT
      ...
      otm.team_role,
      (SELECT EXISTS(SELECT 1 FROM operator_units ou WHERE ou.operator_id = p.id)) AS has_operator_unit
    FROM profiles p
  ...
  LOOP
    ...
    IF r.business_stage <> r.expected_stage THEN
      v_issues := v_issues || jsonb_build_object('type', 'alias_not_normalized', 'current', r.business_stage, 'expected', r.expected_stage);
    END IF;

    IF r.current_phase < r.expected_min_phase THEN
      v_issues := v_issues || jsonb_build_object('type', 'phase_too_low', 'current_phase', r.current_phase, 'expected_min', r.expected_min_phase);
    END IF;

    IF r.business_stage = 'senior_manager' AND r.current_phase >= 6 AND NOT r.has_operator_unit THEN
      v_issues := v_issues || jsonb_build_object('type', 'l6_missing_operator_unit', 'user', r.full_name);
    END IF;

    IF jsonb_array_length(v_issues) > 0 THEN
      v_result := v_result || jsonb_build_object(
        'user_id', r.id, 'full_name', r.full_name,
        'business_stage', r.business_stage, 'current_phase', r.current_phase,
        'is_test_user', r.is_test_user, 'issues', v_issues
      );
    END IF;
  END LOOP;
