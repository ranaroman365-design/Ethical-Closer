...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('error','user_not_found','evaluated_at', now());
  END IF;
  ...
  LOOP
    ...
    IF v_actual_value IS NULL THEN
      ...
      v_missing_kpis := v_missing_kpis || jsonb_build_object(
        'kpi_key', r.kpi_key, 'required_value', r.threshold_value,
        'actual_value', NULL, 'delta', NULL);
    ELSE
      ...
      ELSE
        v_blockers := v_blockers || jsonb_build_object(
          'category','kpi','code','kpi_below_threshold_'||r.kpi_key,
          'message', format('KPI %s %s %s required, actual %s',
            r.kpi_key, r.threshold_operator, r.threshold_value, v_actual_value),
          'severity','high');
        v_missing_kpis := v_missing_kpis || jsonb_build_object(
          'kpi_key', r.kpi_key, 'required_value', r.threshold_value,
          'actual_value', v_actual_value, 'delta', v_actual_value - r.threshold_value);
      END IF;
    END IF;
  END LOOP;
  ...
  SELECT COUNT(*) INTO v_done_modules FROM public.member_progress
  ...
  v_required_modules_ok := CASE
    ...
    ELSE v_completion_pct >= 85 END;
  SELECT COUNT(*) INTO v_quizzes_passed FROM public.quiz_attempts
  ...
  IF NOT v_required_modules_ok THEN
    v_blockers := v_blockers || jsonb_build_object(
      ...
      'message', format('Academy completion %s%% (%s/%s) — required for L%s',
        ROUND(v_completion_pct,1), v_done_modules, v_total_modules, p_target_level),
      'severity','medium');
  END IF;
  ...
  SELECT COUNT(*) INTO v_scorecard_count FROM public.operator_scorecards
    WHERE operator_email = v_profile.email;

  v_min_sim_calls := CASE WHEN p_target_level <= 2 THEN 3 ELSE 5 END;
  ...
  v_application_pass := (v_sim_calls >= v_min_sim_calls) AND (v_live_calls >= v_min_live_calls);
  v_app_score := LEAST(100,
    CASE WHEN v_min_sim_calls > 0
         THEN (LEAST(v_sim_calls, v_min_sim_calls)::numeric / v_min_sim_calls) * 50 ELSE 50 END
    + CASE WHEN v_min_live_calls > 0
         THEN (LEAST(v_live_calls, v_min_live_calls)::numeric / v_min_live_calls) * 50 ELSE 50 END);
  ...
  v_certification_pass := CASE
    WHEN p_target_level <= 3 THEN true
    ELSE COALESCE(v_profile.certified, false) END;
  ...
  INSERT INTO public.promotion_evaluations (
    user_id, current_level, target_level, show_rate, close_rate, earnings_per_call,
    threshold_passed, evaluation_reason, readiness_score, all_pass, blockers_count,
    evaluation_payload, evaluator
  ) VALUES (
    p_user_id,
    NULLIF(regexp_replace(COALESCE(v_rop.current_level,''),'\D','','g'),'')::int,
    p_target_level,
    v_kpis.show_rate, v_kpis.closing_rate, v_kpis.earnings_per_call,
    (v_result->>'all_pass')::boolean,
    'canonical_v1 shadow evaluation',
    v_score, (v_result->>'all_pass')::boolean,
    jsonb_array_length(v_blockers), v_result, 'canonical_v1'
  );
...
GRANT EXECUTE ON FUNCTION public.evaluate_promotion_canonical(uuid, integer) TO anon, authenticated, service_role;