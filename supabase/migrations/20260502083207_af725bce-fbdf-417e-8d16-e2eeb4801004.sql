...
BEGIN
  ...
  -- PAYOUT PHASE — Closer commission
  IF v_locked_closer_id IS NOT NULL THEN
    ...
    -- Use business_stage (the actual human-readable stage)
    SELECT business_stage INTO v_user_stage FROM profiles WHERE id = v_locked_closer_id;

    IF v_config IS NOT NULL AND v_user_stage IS NOT NULL THEN
      -- Try closer sub-key first, then flat rates structure
      v_rate := COALESCE(
        (v_config->'commission_rates'->'closer'->>v_user_stage)::numeric,
        (v_config->'commission_rates'->>v_user_stage)::numeric,
        (v_config->'commission_rates'->'closer'->>'default')::numeric,
        (v_config->'commission_rates'->>'default')::numeric,
        0
      );
      ...
      IF v_rate > 0 THEN
        ...
        INSERT INTO commissions (
          ...
          jsonb_build_object('locked_closer_id', v_locked_closer_id, 'original_handler_id', v_original_handler_id, 'product_key', v_product_key, 'rate', v_rate, 'stage', v_user_stage)
        );
        ...
        BEGIN
          ...
          IF v_setter_id IS NOT NULL AND v_setter_id IS DISTINCT FROM v_locked_closer_id THEN
            ...
            v_setter_rate := COALESCE(
              (v_config->'commission_rates'->'setter'->>v_setter_stage)::numeric,
              (v_config->'commission_rates'->>v_setter_stage)::numeric,
              (v_config->'commission_rates'->'setter'->>'default')::numeric,
              0
            );
            IF v_setter_rate > 0 THEN
              v_setter_amount := round(v_revenue * v_setter_rate, 2);
              INSERT INTO commissions (
                call_id, user_id, role, amount, source_type, payout_status, closer_lock_snapshot
              ) VALUES (
                p_call_id, v_setter_id, 'setter', v_setter_amount, 'payment', 'pending',
                jsonb_build_object('setter_id', v_setter_id, 'rate', v_setter_rate, 'stage', v_setter_stage)
              );
