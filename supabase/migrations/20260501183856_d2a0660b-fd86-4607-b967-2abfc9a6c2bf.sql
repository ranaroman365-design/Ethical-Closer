...
BEGIN
  ...
  close_counts AS (
    SELECT 
      COALESCE(rl.closer_id, rl.owner_id) AS op_id,
      COUNT(*) AS cnt,
      SUM(COALESCE(rl.deal_value, 0)) AS rev
    FROM real_leads_view rl
    WHERE rl.created_at >= v_cutoff
      AND (rl.outcome = 'won' OR rl.payment_status = 'paid')
    GROUP BY 1
  ),
  weekly AS (
    SELECT COALESCE(rl.closer_id, rl.owner_id) AS op_id,
      EXTRACT(WEEK FROM rl.created_at) AS wk,
      COUNT(*) AS cnt
    FROM real_leads_view rl
    WHERE rl.created_at >= v_cutoff
      AND (rl.outcome = 'won' OR rl.payment_status = 'paid')
  ...
  activity AS (
    SELECT COALESCE(rl.closer_id, rl.owner_id) AS op_id,
      COUNT(*) FILTER (WHERE rl.created_at >= now() - interval '7 days') AS last7
    FROM real_leads_view rl
    WHERE rl.created_at >= v_cutoff
    GROUP BY 1
  ),
  ...
  sparkline_cte AS (
    ...
    FROM (
      SELECT COALESCE(rl.closer_id, rl.owner_id) AS op_user,
        d::date AS d,
        SUM(COALESCE(rl.deal_value, 0)) AS day_rev
      FROM generate_series(v_cutoff_14d, now(), '1 day') d
      LEFT JOIN real_leads_view rl 
        ON rl.created_at::date = d::date
        AND (rl.outcome = 'won' OR rl.payment_status = 'paid')
      GROUP BY COALESCE(rl.closer_id, rl.owner_id), d::date
    ) c_agg
