...
RETURNS TABLE (
  lead_id uuid, name text, email text, phone text, source text, funnel_id text,
  current_stage text, created_at timestamptz, lead_score integer,
  quiz_status text, has_booking boolean, appointment_at timestamptz,
  setter_id uuid, setter_name text, closer_id uuid, closer_name text,
  attendance_status text, call_outcome text, revenue_status text,
  deal_value numeric, payment_status text, heat text, total_count bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
