...
-- Rewrite build_operator_scorecards to use canonical revenue source (calls table)
-- instead of call_outcomes, matching ceo_dashboard_snapshot's data hierarchy.
...
BEGIN
  ...
  -- (same tables used by ceo_dashboard_snapshot setter_stats CTE)
  -- ══════════════════════════════════════════════════════════════
  FOR r IN
    SELECT
      p.id AS user_id,
  ...
  -- ══════════════════════════════════════════════════════════════
  -- CLOSER SCORECARDS — canonical source: calls table
  -- Uses calls.result = 'won' and calls.revenue
  -- (SAME source as ceo_dashboard_snapshot closer_stats CTE)
  -- Previously used call_outcomes — now aligned.
  -- ══════════════════════════════════════════════════════════════
  FOR r IN
    SELECT
      c.user_id,
      ...
      COALESCE(SUM(c.revenue) FILTER (WHERE c.result = 'won' AND c.created_at >= v_start AND c.is_simulation = false), 0) AS revenue
    FROM calls c
