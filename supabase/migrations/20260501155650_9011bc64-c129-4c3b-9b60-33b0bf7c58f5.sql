...
BEGIN
  ...
  -- Count
  EXECUTE format(
    'SELECT count(*) FROM public.leads l WHERE l.created_at >= $1 AND l.is_simulation = false
     AND (%s)
  ...
  INTO v_rows
  USING v_since, p_scope, p_user_id, p_team_ids, p_funnel, p_source, p_heat, p_search, p_page_size, v_offset;
