-- Calendar Deep-Link · single aggregated context endpoint
-- Returns appointment + lead + counts. RLS on underlying tables continues to apply.
...
BEGIN
  ...
  IF v_lead.id IS NOT NULL THEN
    SELECT COUNT(*) INTO v_calls_count FROM calls WHERE user_id = v_lead.id OR lead_id = v_lead.id;
    ...
    SELECT to_jsonb(c.*) INTO v_last_call FROM calls c
      WHERE c.lead_id = v_lead.id ORDER BY c.created_at DESC LIMIT 1;
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  -- fail soft so RLS denials don't break UI
  RETURN jsonb_build_object('error', SQLERRM);
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_appointment_full_context(uuid) TO authenticated;