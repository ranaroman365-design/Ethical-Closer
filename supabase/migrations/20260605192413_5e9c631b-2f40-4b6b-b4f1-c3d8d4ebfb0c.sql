...
DECLARE
  ...
  v_setter_pipeline_stages text[] := ARRAY[
    ...
    'setter_no_response','setter_qualified','backlog'
  ];
BEGIN
  ...
  IF v_existing_active_id IS NOT NULL THEN
    ...
    RETURN jsonb_build_object(
      'success', false, 'error_code', 'ACTIVE_APPOINTMENT_EXISTS',
      ...
      'existing_status', v_existing_status, 'can_reschedule', true
    );
  END IF;
  ...
  -- ═══ STATE-MACHINE-AWARE STAGE ROUTING ═══
  v_target_stage := CASE
  ...
  v_target_state := CASE
    WHEN v_lead_state = 'engaged' THEN 'booked'
    ELSE v_lead_state
  END;

  -- Walk setter-pipeline → setter_booked via the only legal intermediate (setter_qualified)
  IF v_target_stage = 'setter_booked'
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      VALUES ('lead_stage_walk_failed', 'create_manual_appointment',
              format('lead=%s appt=%s from=%s -> setter_booked failed: %s',
                     p_lead_id, v_appointment_id, v_lead_stage, SQLERRM),
              jsonb_build_object('lead_id', p_lead_id, 'appointment_id', v_appointment_id,
                                 'previous_stage', v_lead_stage),
              jsonb_build_object('new_stage', 'setter_booked'),
              now());
      v_target_stage := v_lead_stage;
    END;
  END IF;
  ...
  BEGIN
    UPDATE public.leads SET
      ...
      stage = CASE
                WHEN v_target_stage = 'setter_booked' THEN stage  -- already walked
                ELSE v_target_stage
              END,
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    VALUES ('lead_transition_rejected', 'create_manual_appointment',
            format('lead=%s appt=%s stage=%s->%s state=%s->%s rejected: %s',
                   p_lead_id, v_appointment_id, v_lead_stage, v_target_stage,
                   v_lead_state, v_target_state, SQLERRM),
            jsonb_build_object('lead_id', p_lead_id, 'appointment_id', v_appointment_id,
                               'previous_stage', v_lead_stage, 'previous_state', v_lead_state),
            jsonb_build_object('new_stage', v_target_stage, 'new_state', v_target_state),
            now());
  ...
  -- ═══ SUCCESS-PATH AUDIT LOG ═══
  SELECT COALESCE(stage,'') INTO v_final_stage FROM public.leads WHERE id = p_lead_id;

  INSERT INTO audit_logs (action, source_type, note, before_state, after_state, created_at)
  VALUES (
    'manual_appointment_created',
    'create_manual_appointment',
    format('lead=%s appt=%s stage %s -> %s', p_lead_id, v_appointment_id, v_lead_stage, v_final_stage),
    jsonb_build_object('lead_id', p_lead_id, 'previous_stage', v_lead_stage),
    jsonb_build_object('appointment_id', v_appointment_id, 'new_stage', v_final_stage),
    now()
  );
