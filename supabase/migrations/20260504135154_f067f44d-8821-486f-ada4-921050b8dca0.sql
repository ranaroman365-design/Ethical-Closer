-- Temporary launch stabilization for calendar RPCs.
-- Keeps existing functions, but removes restrictive internal gates that still blocked the UI after RLS was relaxed.

CREATE OR REPLACE FUNCTION public.create_manual_appointment(
  p_lead_id uuid,
  p_starts_at timestamp with time zone,
  p_ends_at timestamp with time zone,
  p_call_type text DEFAULT 'setter_call'::text,
...
BEGIN
  ...
  INSERT INTO public.appointments (
    ...
    p_lead_id, p_starts_at, p_ends_at, COALESCE(NULLIF(p_call_type, ''), 'setter_call'),
  ...
  INSERT INTO public.calendar_events (
    ...
    jsonb_build_object(
      ...
      'call_type', COALESCE(NULLIF(p_call_type, ''), 'setter_call'),
      'closer_id', p_closer_id,
      'setter_notes', p_setter_notes,
      'temporary_launch_access', true
    )
  );
  ...
  VALUES (
    ...
    END,
    jsonb_build_object('appointment_id', v_appointment_id, 'closer_id', p_closer_id, 'call_type', COALESCE(NULLIF(p_call_type, ''), 'setter_call'))
  );
...
RETURNS TABLE(id uuid, name text, email text, phone text, quiz_score integer, lead_quality text, source text, stage text, created_at timestamp with time zone)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  ...
  FROM public.leads l
  WHERE COALESCE(l.has_booking, false) = false
