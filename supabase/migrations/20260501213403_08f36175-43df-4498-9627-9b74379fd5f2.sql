...
-- Fix 1: compute_talent_scores — use real_appointments_view for booking/show counts
...
BEGIN
  ...
  -- FIXED: use real_appointments_view instead of raw appointments
  booking_counts AS (
...
-- Fix 2: get_operator_unit_performance — divide payment_links amount by 100 (cents→EUR)
CREATE OR REPLACE FUNCTION public.get_operator_unit_performance(p_range_days integer DEFAULT 30)
RETURNS TABLE(
  ...
  total_closes bigint, total_revenue numeric,
  close_rate numeric, show_rate numeric, is_setup_pending boolean
)
