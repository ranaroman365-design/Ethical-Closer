-- Fix type cast bug in evaluate_user_for_promotion: record_id is uuid, not text
...
DECLARE
  ...
  v_level_entry jsonb;
BEGIN
  -- Fixed: cast calls.id to text to match record_id type, OR compare as uuid
  SELECT EXISTS (
    SELECT 1 FROM data_integrity_logs
    WHERE record_id IN (SELECT id FROM calls WHERE user_id = p_user_id)
