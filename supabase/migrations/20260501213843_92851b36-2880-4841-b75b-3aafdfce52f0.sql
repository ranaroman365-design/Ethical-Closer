...
CREATE FUNCTION public.compute_talent_scores(
...
BEGIN
  ...
  -- Query leads table directly with test filter (same logic as real_leads_view)
  filtered_leads AS (
    SELECT l.id, l.created_at, l.name, l.source, l.owner_id, l.setter_id, l.closer_id,
           l.outcome, l.payment_status, l.is_simulation
    FROM leads l
    WHERE l.created_at >= v_cutoff
      ...
      AND l.name NOT ILIKE 'test%' AND l.name NOT ILIKE '%kein lead%' AND l.name NOT ILIKE '%test test%'
  ),
  lead_counts AS (
    SELECT COALESCE(fl.owner_id, fl.setter_id, fl.closer_id) AS op_id, COUNT(*) AS cnt
    FROM filtered_leads fl GROUP BY 1
  ),
  -- Query appointments directly with test-lead exclusion
  booking_counts AS (
